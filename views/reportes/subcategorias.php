<div class="container py-4">
   <div class="card shadow-sm mb-4">
       <div class="card-body">
           <h2 class="card-title text-primary"><i class="bi bi-folder-2-open"></i> Reportes de subcategorias</h2>
           <form method="GET" action="/trucksisx/reportes.php" class="row g-3 align-items-center mb-3">
                 <input type="hidden" name="reporte" value="subcategorias">
                 <div class="col-auto">
                 ...
                 <div class="col-auto">
                    <input type="text" id="nombre" name="nombre" class="form-control" placeholder="Nombre de la subcategoría" value="<?= htmlspecialchars($_GET['nombre'] ?? '') ?>">
                 </div>
                 <div class="col-auto">
                        <button type="submit" class="btn btn-primary">Filtrar</button>
                 </div>
                <div class="col-auto">
                          <a href="/trucksisx/reportes.php?reporte=subcategorias" class="btn btn-primary">Reporte total</a>
                </div>
           </form>
           <div class="mb-3">
                        ...
                        <?php if (!empty($subcategorias)): ?>
                            <?php foreach ($subcategorias as $sub): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($sub['caracteristicas']); ?></td>
...
<script>
    document.getElementById('descargar-pdf').onclick = function(e) {
        ...
        let url = '/trucksisx/reportes.php?reporte=descargarSubcategoriasPDF';
    ...
    document.getElementById('descargar-excel').onclick = function(e) {
        ...
        const nombre = document.getElementById('nombre').value;
        const categoria = document.getElementById('categoria').value;
        let url = '/trucksisx/reportes.php?reporte=descargarSubcategoriasExcel';
        if (categoria) url += '&categoria=' + encodeURIComponent(categoria);
        if (nombre) url += '&nombre=' + encodeURIComponent(nombre);
        window.location.href = url;
    };
