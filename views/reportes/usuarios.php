<div class="container mt-4">
   <div class="card shadow-sm mb-4">
       <div class="card-body">
           <h2 class="card-title text-primary"><i class="bi bi-people"></i> Reportes de usuarios</h2>
           <form method="GET" action="/trucksisx/reportes.php" class="row g-3 align-items-center mb-3">
                    ...
                    <select id="rol" name="rol" class="form-select">
                        ...
                        <?php if (!empty($roles)): ?>
                            <?php foreach ($roles as $rol): ?>
                                <option value="<?= htmlspecialchars($rol['rol']) ?>" <?= (isset($_GET['rol']) && $_GET['rol'] == $rol['rol']) ? 'selected' : ''; ?>>
                                    <?= htmlspecialchars(ucfirst($rol['rol'])); ?>
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
                ...
                <div class="col-auto">
                          <a href="/trucksisx/reportes.php?reporte=usuarios" class="btn btn-primary">Reporte total</a>
                </div>
           </form>
           <div class="mb-3">
...
<script>
    document.getElementById('descargar-pdf').onclick = function(e) {
        ...
        let url = '/trucksisx/reportes.php?reporte=descargarUsuariosPDF';
    ...
    document.getElementById('descargar-excel').onclick = function(e) {
        ...
        let url = '/trucksisx/reportes.php?reporte=descargarUsuariosExcel';
        if (usuario) url += '&usuario=' + encodeURIComponent(usuario);
        if (rol) url += '&rol=' + encodeURIComponent(rol);
