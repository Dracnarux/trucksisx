<div class="container mt-4">
   <div class="card shadow-sm mb-4">
       <div class="card-body">
           <h2 class="card-title text-primary"><i class="bi bi-clipboard-check"></i> Reportes de Órdenes de Trabajo</h2>
           <form method="GET" action="/trucksisx/reportes.php" class="row g-3 align-items-center mb-3">
                 <input type="hidden" name="reporte" value="ordenesTrabajo">
                 <div class="col-md-3">
                    ...
                    <select id="tecnico" name="tecnico" class="form-select">
                        ...
                        <?php if (!empty($tecnicos)): ?>
                            <?php foreach ($tecnicos as $tec): ?>
                                <option value="<?= htmlspecialchars($tec['id']) ?>" <?= (isset($_GET['tecnico']) && $_GET['tecnico'] == $tec['id']) ? 'selected' : ''; ?>>
                 ...
                 <div class="col-md-12 d-flex gap-2 align-items-end">
                        <button type="submit" class="btn btn-primary">Filtrar</button>
                          <a href="/trucksisx/reportes.php?reporte=ordenesTrabajo" class="btn btn-primary">Reporte total</a>
                 </div>
           </form>
           <div class="mb-3">
           ...
           <div id="tabla-ordenes" class="table-responsive">
                    ...
                    <tbody>
                        <?php if (!empty($ordenes)): ?>
                            <?php foreach ($ordenes as $orden): ?>
                                <tr>
                                    <td><?= htmlspecialchars($orden['id_orden'] ?? $orden['id'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($orden['id_vehiculo'] ?? $orden['vehiculo_placa'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($orden['descripcion'] ?? $orden['nombre_trabajo'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($orden['fecha_inicio'] ?? $orden['fecha_creacion'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($orden['fecha_fin'] ?? $orden['fecha_estimada'] ?? 'N/A'); ?></td>
                                    <td><?= htmlspecialchars($orden['estado'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($orden['id_tecnico'] ?? $orden['tecnico_nombre'] ?? ''); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
