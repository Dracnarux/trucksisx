<div class="container mt-4">
   <div class="card shadow-sm mb-4">
       <div class="card-body">
           <h2 class="card-title text-primary"><i class="bi bi-box-arrow-right"></i> Reportes de Salidas de Repuestos</h2>
           <form method="GET" action="/trucksisx/reportes.php" class="row g-3 align-items-center mb-3">
                 <input type="hidden" name="reporte" value="salidasRepuestos">
                 <div class="col-md-3">
                    <label for="repuesto" class="form-label">Repuesto</label>
                    <select id="repuesto" name="repuesto" class="form-select">
                        <option value="">Todos</option>
                        <?php if (!empty($repuestos)): ?>
                            <?php foreach ($repuestos as $rep): ?>
                                <option value="<?= htmlspecialchars($rep['id']) ?>" <?= (isset($_GET['repuesto']) && $_GET['repuesto'] == $rep['id']) ? 'selected' : ''; ?>>
                 ...
                 <div class="col-md-12 d-flex gap-2 align-items-end">
                        <button type="submit" class="btn btn-primary">Filtrar</button>
                          <a href="/trucksisx/reportes.php?reporte=salidasRepuestos" class="btn btn-primary">Reporte total</a>
                 </div>
           </form>
           <div class="mb-3">
                        ...
                        <?php if (!empty($salidas)): ?>
                            <?php foreach ($salidas as $sal): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($sal['id_repuesto'] ?? $sal['repuesto_nombre'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($sal['id_vehiculo'] ?? $sal['vehiculo_placa'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($sal['cantidad'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($sal['fecha_salida'] ?? $sal['fecha'] ?? ''); ?></td>
                                    <td><?= htmlspecialchars($sal['autorizado_por'] ?? ''); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
