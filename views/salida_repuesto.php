        ...
        ordenSelect.addEventListener('change', function() {
            ...
            fetch(`../api/orden_trabajo_detalles.php?id=${ordenId}`)
                ...
                .then(data => {
                    ...
                    // Rellenar alerta automáticamente
                    if (alertaSelect && data.alerta_id) {
                        // Buscar si la opción existe en el select
                        const alertaOption = alertaSelect.querySelector(`option[value="${data.alerta_id}"]`);
                        if (alertaOption) {
    ...
    fetch(url)
        ...
        .then(repuestos => {
            ...
            if (repuestos.length === 0) {
                html += '<option value="" disabled>No hay repuestos disponibles</option>';
            } else {
...
function mostrarInfoOrdenTrabajo(data) {
    ...
    if (data.vehiculo_id) {
        ...
        document.getElementById('info-vehiculo').textContent = vehiculoInfo;
        document.getElementById('info-vehiculo-container').style.display = 'block';
        hasInfo = true;
    } else {
        document.getElementById('info-vehiculo-container').style.display = 'none';
    }
    
    // Conductor
    if (data.conductor_nombre_completo) {
        document.getElementById('info-conductor').textContent = data.conductor_nombre_completo;
        document.getElementById('info-conductor-container').style.display = 'block';
        hasInfo = true;
    } else {
        document.getElementById('info-conductor-container').style.display = 'none';
    }
    ...
    if (data.tipo_alerta) {
        ...
        document.getElementById('info-tipo-alerta').textContent = tipoAlertaLabel[data.tipo_alerta] || data.tipo_alerta;
        document.getElementById('info-tipo-alerta-container').style.display = 'block';
        hasInfo = true;
    } else {
        document.getElementById('info-tipo-alerta-container').style.display = 'none';
    }
    
    // Prioridad
    if (data.alerta_prioridad) {
        const prioridadHTML = {
        ...
        document.getElementById('info-prioridad').innerHTML = prioridadHTML[data.alerta_prioridad] || data.alerta_prioridad;
        document.getElementById('info-prioridad-container').style.display = 'block';
        hasInfo = true;
    } else {
        document.getElementById('info-prioridad-container').style.display = 'none';
    }
    
    // Repuesto especificado en OT
    if (data.nombre_repuesto) {
        document.getElementById('info-repuesto-ot').textContent = data.nombre_repuesto;
        document.getElementById('info-repuesto-ot-container').style.display = 'block';
        hasInfo = true;
    } else {
        document.getElementById('info-repuesto-ot-container').style.display = 'none';
    }
