...
if (!$rol_conductor && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax']) && $_POST['ajax'] === '1') {
    ...
    if (!empty($_POST['id'])) {
        // Obtener la asignación anterior para limpiar vehículo previo
        $sql_prev = "SELECT regis_vehic_id FROM cond WHERE id = ?";
        $stmt_prev = $conn->prepare($sql_prev);
        $stmt_prev->bind_param('i', $_POST['id']);
        $stmt_prev->execute();
        ...
        if ($conductor_actualizado) {
            // Sincronizar tabla regis_vehic
            $conductor_id = intval($_POST['id']);
            $vehiculo_nuevo = !empty($_POST['regis_vehic_id']) ? intval($_POST['regis_vehic_id']) : null;
            
            // 1. Limpiar vehículo anterior si existía y es diferente al nuevo
            if ($vehiculo_anterior && $vehiculo_anterior != $vehiculo_nuevo) {
                $sql_clean_prev = "UPDATE regis_vehic SET cond_id = NULL, estado = 'Sin conductor' WHERE id = ?";
                $stmt_clean_prev = $conn->prepare($sql_clean_prev);
                $stmt_clean_prev->bind_param('i', $vehiculo_anterior);
                $stmt_clean_prev->execute();
                error_log("cond.php: Limpiado vehículo anterior $vehiculo_anterior del conductor $conductor_id");
            }
            
            // 2. Asignar nuevo vehículo si se especificó
            if ($vehiculo_nuevo) {
                // Limpiar cualquier asignación previa de este vehículo a otros conductores
                $sql_clean_other = "UPDATE regis_vehic SET cond_id = NULL, estado = 'Sin conductor' WHERE id = ? AND cond_id != ?";
                $stmt_clean_other = $conn->prepare($sql_clean_other);
                $stmt_clean_other->bind_param('ii', $vehiculo_nuevo, $conductor_id);
                $stmt_clean_other->execute();
                
                // Asignar el vehículo al conductor
                $sql_assign_new = "UPDATE regis_vehic SET cond_id = ?, estado = 'Asignado' WHERE id = ?";
                $stmt_assign_new = $conn->prepare($sql_assign_new);
                $stmt_assign_new->bind_param('ii', $conductor_id, $vehiculo_nuevo);
                $vehiculo_asignado = $stmt_assign_new->execute();
                
                if ($vehiculo_asignado) {
                    error_log("cond.php: Vehículo $vehiculo_nuevo asignado exitosamente al conductor $conductor_id");
                } else {
                    error_log("cond.php: Error al asignar vehículo $vehiculo_nuevo al conductor $conductor_id");
                }
            }
        }
